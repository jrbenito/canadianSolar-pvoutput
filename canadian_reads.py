...
class Inverter(object):
    ...
    def __init__(self, address, port):
        ...
        self.inv = ModbusClient(method='rtu', port=port, baudrate=9600, stopbits=1, parity='N', bytesize=8, timeout=1)
        self.unit = address
    ...
    def read_inputs(self):
        ...
        if self.inv.connect():
            ...
            if not rr.isError():
                ...
                self.wh_today = float((rr.registers[26] << 16)+rr.registers[27])*100 # inv report 0.1 KWh
                self.wh_total = float((rr.registers[28] << 16)+rr.registers[29])*100 # inv report 0.1 KWh
    ...
    def version(self):
        ...
        if self.inv.connect():
            ...
            if not rr.isError():
                ...
                self.firmware = str(chr(rr.registers[9]>>8)+chr(rr.registers[9]&0x000000FF)  \
                ...
                self.control_fw = str(chr(rr.registers[12]>>8)+chr(rr.registers[12]&0x000000FF) \
                ...
                self.serial_no =  str(chr(rr.registers[23]>>8)+chr(rr.registers[23]&0x000000FF) \
                ...
                self.model_no = 'T' + str((mo&0XF00000)>>20) + ' Q' + str((mo&0X0F0000)>>16) + \
